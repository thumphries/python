...
# offered several problems during programming contests. Long before
# the start the friends decided that they will implement a problem if
# at least two of them are sure about the solution. Otherwise, the
...
# Note
#
# In the first sample Petya and Vasya are sure that they know how to
# solve the first problem and all three of them know how to solve the
# second problem. That means that they will write solutions for these
# problems. Only Petya is sure about the solution for the third
# problem, but that isn't enough, so the friends won't take it.
#  
#
...
problems = int(sys.stdin.readline().rstrip())

solved = 0
for n in range(problems):
    sure = sum(map(int, sys.stdin.readline().rstrip().split()))
    if (sure > 1):
        solved = solved + 1
